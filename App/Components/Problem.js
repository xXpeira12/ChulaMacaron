...
export default function Problem() {
  ...
  const problems = [
    ...
    { label: 'ต้นไม้', value: 'ต้นไม้', list: ['ต้นไม้ตาย', 'กิ่งไม้หัก', 'กิ่งไม้บดบังทัศนวิสัย', 'กิ่งไม้พันสายไฟ', 'ต้นไม้กัดกั้นทางเดิน'] },
    ...
    { label: 'อื่น ๆ', value: 'อื่น ๆ', list: ['ระบุรายละเอียดปัญหา'] }
  ];
      ...
      {problems.map((problem, index) => (
        <View key={index}>
          <TouchableOpacity onPress={() => setSelectedProblemIndex(index === selectedProblemIndex ? null : index)}>
            <Text style={{fontFamily: 'chulaBold', fontSize:20, color:'#E26199',paddingTop:10}}>{problem.value} ▾ </Text>
          </TouchableOpacity>

          {selectedProblemIndex === index && problem.list.map((list, index) => (
            <View key={index} style={{marginLeft:20}}>
              <Text style={{fontFamily: 'chulaReg', fontSize:20, color:'#E26199',paddingTop:10}}>- {list}</Text>
            </View>
          ))}
        </View>
      ))}
